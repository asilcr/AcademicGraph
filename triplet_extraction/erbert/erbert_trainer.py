...
from erbert import models, prediction
from erbert import sampling
from erbert import util
from erbert.entities import Dataset
from erbert.evaluator import Evaluator
from erbert.input_reader import JsonInputReader, BaseInputReader
from erbert.loss import erbertLoss, Loss
from tqdm import tqdm
from erbert.trainer import BaseTrainer

SCRIPT_PATH = os.path.dirname(os.path.realpath(__file__))


class erbertTrainer(BaseTrainer):
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: Type[BaseInputReader]):
        ...
        # erbert is currently optimized on a single GPU and not thoroughly tested in a multi GPU setup
        # If you still want to train erbert on multiple GPUs, uncomment the following lines
        # # parallelize model
        ...
        compute_loss = erbertLoss(rel_criterion, entity_criterion, model, optimizer, scheduler, args.max_grad_norm)
    ...
    def _load_model(self, input_reader):
        ...
        config.erbert_version = model_class.VERSION
        model = model_class.from_pretrained(self._args.model_path,
                                            config=config,
                                            # erbert model parameters
