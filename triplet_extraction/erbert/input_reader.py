...
from erbert.entities import Dataset, EntityType, RelationType, Entity, Relation, Document
from erbert.opt import spacy
...
class JsonInputReader(BaseInputReader):
    ...
    # 从数据集中提供的head和tail到'entities'里面找到对应的实体，
    # 自动转化relation中的实体的head和tail最终得到一个句子中一对实体的第一个和第二个实体
    def _parse_relations(self, jrelations, entities, dataset) -> List[Relation]:
        ...
        for jrelation in jrelations:
            ...
            reverse = int(tail.tokens[0].index) < int(head.tokens[0].index) # 判断是否调换顺序
...
def _parse_tokens(jtokens, dataset, tokenizer):
    ...
    # parse tokens
    for i, token_phrase in enumerate(jtokens): # token为list
...
"""
样例数据：
{'tokens': ['Eaglebrook', 'East', 'opened', 'in', 'Middletown', 'in', 'April', '.'],
 'entities': [{'type': 'Org', 'start': 0, 'end': 2}, {'type': 'Loc', 'start': 4, 'end': 5}], 
'relations': [{'type': 'OrgBased_In', 'head': 0, 'tail': 1}], 'orig_id': 1452}
"""