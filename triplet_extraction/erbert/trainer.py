...
from erbert.opt import tensorboardX
...
class BaseTrainer:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        if hasattr(args, 'save_path'):
            ...
            self._save_path = os.path.join(self._args.save_path, self._args.label,current_timestamp)

            # self._save_path="r"+self._save_path.replace("\","\\")
        ...
        if hasattr(args, 'log_path'):
            ...
            self._log_path = os.path.join(self._args.log_path, self._args.label, current_timestamp)
    ...
    def _save_model(self, save_path: str, model: PreTrainedModel, tokenizer: PreTrainedTokenizer,
        ...
        if isinstance(model, DataParallel):
            model.module.save_pretrained(dir_path,safe=False)
        else:
            model.save_pretrained(dir_path,safe=False)

        # save vocabulary
        tokenizer.save_pretrained(dir_path,safe=False)
