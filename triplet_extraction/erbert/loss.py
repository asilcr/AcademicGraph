...
class erbertLoss(Loss):
    ...
    def compute(self, entity_logits, rel_logits, entity_types, rel_types, entity_sample_masks, rel_sample_masks):
        # entity loss
        entity_logits = entity_logits.view(-1, entity_logits.shape[-1])  # [batch_size*entity_num_in_seq,entity_type_num]
        entity_types = entity_types.view(-1)                             # [batch_size*entity_num_in_seq]                       
        ...
        if rel_count.item() != 0:
            rel_logits = rel_logits.view(-1, rel_logits.shape[-1])  #  [batch_size*relation_num_in_seq,relation_type_num]
            rel_types = rel_types.view(-1, rel_types.shape[-1])     #  [batch_size*relation_num_in_seq,relation_type_num]
